...
class ExperimentEVENT(object):
    ...
    def __init__(self,
        ...
        self.result_dir = os.path.join(result_dir, 'EVENT')
        self.report_dir = os.path.join(report_dir, 'EVENT')
        self.nbins_iou = 101
        self.repetitions = 3
    ...
    def run(self,
        ...
        print('Running tracker %s on EVENT DATASET...' % tracker.name)
        ...
        for s in range(start_idx, end_idx):
            # if s >= 1:
            #     break
            img_files_pos, img_files_neg, anno = self.dataset[s]
            ...
            for r in range(self.repetitions):
                ...
                # tracking loop
                boxes, times = tracker.track(img_files_pos,
                                             img_files_neg,
                                             anno[0, :],
                                             visualize=visualize)
            ...
            if save_video:
                ...
                image = Image.open(img_files_pos[0])
                img_W, img_H = image.size
                out_video = cv2.VideoWriter(video_file,
                                            cv2.VideoWriter_fourcc(*'MJPG'), 10,
                                            (img_W, img_H))
                for ith, (img_file, pred) in enumerate(zip(img_files_pos, boxes)):
        ...
        if self.subset == 'test':
            ...
            for name in tracker_names:
                ...
                for s, (_, anno, meta) in enumerate(self.dataset):
                    ...
                    bound = ast.literal_eval('(346, 260)')
                    seq_ious = [
