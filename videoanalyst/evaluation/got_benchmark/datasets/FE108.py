...
class FE108(object):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            ...
            seq_name = index
        img_files = FE108.data_dict[self.subset][seq_name]["img_files"]
        anno = FE108.data_dict[self.subset][seq_name]["anno"]
        ...
        if self.return_meta:
            meta = FE108.data_dict[self.subset][seq_name]["meta"]
    ...
    def _ensure_cache(self):
        ...
        """
        # check if subset cache already exists in FE108.data_dict and is valid w.r.t. list.txt
        if self._check_cache_for_current_subset():
            return

        # load subset cache into FE108.data_dict
    ...
    def _check_cache_for_current_subset(self) -> bool:
        r""" check if FE108.data_dict[subset] exists and contains all record in seq_names
        """
        is_valid_data_dict = (self.subset in FE108.data_dict) and \
                             (set(FE108.data_dict[self.subset].keys()) == set(self.seq_names))
    ...
    def _load_cache_for_current_subset(self, cache_path: str):
        ...
        with open(cache_path, "rb") as f:
            FE108.data_dict[self.subset] = pickle.load(f)
        logger.info("{}: loaded cache file {}".format(FE108.__name__,
                                                      cache_path))
    ...
    def _build_cache_for_current_subset(self):
        ...
        logger.info("{}: start loading subset {}".format(
            FE108.__name__, subset))
        for seq_name in tqdm(self.seq_names):
            seq_dir = os.path.join(root_dir, subset, seq_name)
            img_files, anno, meta = self.load_single_sequence(seq_dir)
            FE108.data_dict[self.subset][seq_name] = dict(img_files=img_files,
                                                           anno=anno,
                                                           meta=meta)
        with open(self.cache_path, "wb") as f:
            pickle.dump(FE108.data_dict[self.subset], f)
        logger.info("{}: dump cache file to {}".format(FE108.__name__,
                                                       self.cache_path))
