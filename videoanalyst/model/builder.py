...
from .transfor import builder as transfor_builder
...
from .attention import builder as atten_builder
...
def build(
    ...
    if task == "track":
        ...
        transfor = transfor_builder.build(task, cfg.transfor)
        atten = atten_builder.build(task, cfg.atten)
        head = head_builder.build(task, cfg.task_head)
        losses = loss_builder.build(task, cfg.losses)
        task_model = task_builder.build(task, cfg.task_model, backbone, transfor, atten, head,
                                        losses)
        # task_model = task_builder.build(task, cfg.task_model, backbone, transfor, head,
        #                                 losses)
        print(task_model)
    elif task == "vos":
...
def get_config(task_list: List) -> Dict[str, CfgNode]:
    ...
    for task in cfg_dict:
        ...
        cfg["transfor"] = transfor_builder.get_config(task_list)[task]
        cfg["atten"] = atten_builder.get_config(task_list)[task]
