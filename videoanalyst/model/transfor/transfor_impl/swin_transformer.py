...
class SwinTransformer(ModuleBase):
    ...
    def forward(self, img_pos, img_neg):
        # max_pos = torch.where(img_pos[0] > img_pos[1], img_pos[0], img_pos[1])
        # max_pos = torch.where(max_pos > img_pos[2], max_pos, img_pos[2])
        # max_pos = torch.where(max_pos > img_pos[3], max_pos, img_pos[3])
        # max_pos = torch.where(max_pos > img_pos[4], max_pos, img_pos[4])
        # max_neg = torch.where(img_neg[0] > img_neg[1], img_neg[0], img_neg[1])
        # max_neg = torch.where(max_neg > img_pos[2], max_neg, img_pos[2])
        # max_neg = torch.where(max_neg > img_pos[3], max_neg, img_pos[3])
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                # outs.append(out)
        out = self.relu(self.bn(self.conv8(out)))
        return None, out
